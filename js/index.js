...
(async function initializeIndexPage() {
  try {
    ...
    console.log(`Página de inicio inicializada en idioma: ${selectedLang}`);
    ...
    try {
      ...
      console.warn(`Fallback aplicado: idioma ${DEFAULT_LANG}`);
      
    } catch (fallbackError) {
      console.error("Error crítico cargando idioma por defecto:", fallbackError);
...
window.addEventListener('error', (event) => {
  console.error('Error no capturado en index:', event.error);
});

// Manejar promesas rechazadas no capturadas
window.addEventListener('unhandledrejection', (event) => {
  console.error('Promesa rechazada no manejada en index:', event.reason);
});